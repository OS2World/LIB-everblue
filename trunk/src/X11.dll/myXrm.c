    ...
    Bool override)
{
	...
	*into = from;
	...
	mutex_lock(from->linfo, FALSE);
	mutex_lock((*into)->linfo, FALSE);
	...
	mutex_close(from->linfo);
	Xfree((char *)from);
	mutex_unlock((*into)->linfo);
    }
}
    ...
    XrmValuePtr		value)
{
    ...
    mutex_lock((*pdb)->linfo, FALSE);
    PutEntry(*pdb, bindings, quarks, type, value);
    mutex_unlock((*pdb)->linfo);
}
...
ReadInFile(_Xconst char *filename)
{
	...
	filename = __XOS2RedirRoot(filename2);
